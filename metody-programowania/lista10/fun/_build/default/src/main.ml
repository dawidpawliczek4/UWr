...
let rec eval_env (env : env) (e : expr) : value =
  ...
  | Ref e -> let v = eval_env env e in let r = ref v in VRef r
  | Deref e -> (match eval_env env e with VRef r -> !r | _ -> failwith "type error: not a reference")
  | Assign (e1, e2) ->
      (match eval_env env e1 with
        VRef r -> let v = eval_env env e2 in r := v; v | _ -> failwith "type error: not a reference")
  | Seq (e1, e2) -> let _ = eval_env env e1 in eval_env env e2
...
(* let e' = parse 
   "let next = let x = ref 0 in fun y -> x := !x + 1; !x in next 0 + next 0" *)