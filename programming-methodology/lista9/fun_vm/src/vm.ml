...
let flatten (e : cmd list) : cmd list =
  let rec flatten' (e : cmd list) (acc : cmd list) : cmd list =
    match e with
    | [] -> acc
    | Lbl l :: e' -> flatten' e' (Lbl l :: acc)
    | Jmp l :: e' -> flatten' e' (Jmp l :: acc)
    | c :: e' -> flatten' e' (c :: acc)
  in
  flatten' e []
