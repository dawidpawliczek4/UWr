...
| [x] ->     
  List.init (max 0 (n - x + 1)) (fun i ->
    (List.init i (fun _ -> false)) @ (List.init x (fun _ -> true)) @ (List.init (n - x - i) (fun _ -> false))
  )
| x:: xs ->
    List.concat_map (fun i ->
      let first_part = (List.init i (fun _ -> false)) @ (List.init x (fun _ -> true)) in
      let rest_len = max 0 (n - x - i - 1) in
      List.map (fun rest -> first_part @ [false] @ rest) (build_row' xs rest_len)
    ) (List.init (n - x - List.length ps + 2) (fun i -> i))
    in build_row' ps n
...
let transpose xss = 
  let rec transpose_aux transposed_matrix = function
    ...
    | matrix ->
      let new_row, remainder_matrix = List.fold_right (fun (head :: tail) (new_row_accumulator, remainder_accumulator) -> 
        (head :: new_row_accumulator, tail :: remainder_accumulator)) matrix ([], []) in 
