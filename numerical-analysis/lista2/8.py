...
def f2(x):
    return (1518 * ( ((2*x)**3 ) / 6 )) / (x**3)

i_values = np.arange(1, 21)
x_values = 10.0 ** (-i_values)

single_precision_results = np.array([f2(np.float32(x)) for x in x_values], dtype=np.float32)

double_precision_results = np.array([f2(np.float64(x)) for x in x_values], dtype=np.float64)
