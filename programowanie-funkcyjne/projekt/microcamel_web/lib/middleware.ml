...
let session_middleware store handler =
  fun req body params->
    ...
    let* (new_id, session_data) =
      match session_id with
      | Some id ->
          let* data = Session.In_memory_store.get store id in
          (match data with
          | Some d -> Lwt.return (id, d)
          | None -> Session.In_memory_store.create_session store)
