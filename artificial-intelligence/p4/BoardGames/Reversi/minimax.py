...
# -*- coding: UTF-8 -*-
'''
Agent alpha-beta do gry w reversi
'''
import random
import sys
...
class Player(object):
    ...
    def loop(self):
        ...
        while True:
            ...
            # we choose best instead of random
            # we prioritise corner moves
            

            moves = self.game.moves(self.my_player)
            better_moves = list(set(moves) & CORNERS)

            _, move = minimax(self.game, self.my_player, 3, True, float('-inf'), float('inf'))
