...
class Reversi {
   public:
    ...
    int eval(int player) const {
        static const int W[8][8] = {{100, -20, 10, 5, 5, 10, -20, 100},
                                    {-20, -50, -2, -2, -2, -2, -50, -20},
                                    {10, -2, 1, 1, 1, 1, -2, 10},
                                    {5, -2, 1, 1, 1, 1, -2, 5},
                                    {5, -2, 1, 1, 1, 1, -2, 5},
...
Reversi::Move iterativeDeepening(Reversi* game, int me) {
    ...
    for (int depth = 3; depth <= MAXD; ++depth) {
        ...
        for (auto m : moves) {
            futures.emplace_back(async(
                launch::async,
                [game, me, m, depth]() -> pair<int, Reversi::Move> {
                    Reversi* child = game->clone();
                    child->doMove(m, me);
...
class AIPlayer {
   ...
   public:
    ...
    Reversi::Move uStart() {
        auto mv = iterativeDeepening(&game, playerId);
