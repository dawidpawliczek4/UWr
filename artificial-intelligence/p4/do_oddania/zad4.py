...
class Jungle:
    ...
    def eval(self, player):
        ...
        den_opp = Jungle.dens[opp]
        max_dist = self.MX + self.MY
        pos_score = 0
        for pl, pieces in self.pieces.items():
            for piece_type, (r, c) in pieces.items():
                value = Jungle.PIECE_VALUES[piece_type]
                # distance from the goal
                dist = abs(r - den_opp[0]) + abs(c - den_opp[1])                
                bonus = max_dist - dist
                if pl == player:
                    pos_score += bonus * value
                else:
                    pos_score -= bonus * value
...
def simulate_games():
    ...
    for i in range(num_games):
        ...
        mcts_agent.my_player = 1
        minmax_agent.my_player = 0
