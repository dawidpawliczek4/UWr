...
//  SORTOWANIE RUCHOW WG HEURY
    // vector<pair<int, Reversi::Move>> sortedMoves;
    // sortedMoves.reserve(moves.size());
    // for (const auto& m : moves) {
    //     Reversi* child = game->clone();
    //     child->doMove(m, player);
    //     int h = child->eval(myPlayer);
    //     delete child;
    //     sortedMoves.emplace_back(h, m);
    // }
    // if (isMax) {
...
class RandomPlayer {
   ...
   public:
    Reversi game;
    RandomPlayer(int id) : playerId(id), rng(12345u) { game = Reversi(); }
...
int main() {
    ...
    for (int i = 0; i < ROUNDS; ++i) {
        ...
        RandomPlayer p2(0);
        auto m1 = p2.uStart();
        auto m2 = p1.nextMove(m1, 0);
        int move_count = 1;

        while (!(m1.first < 0 && m2.first < 0)) {
            m1 = p2.nextMove(m2);
            m2 = p1.nextMove(m1, move_count);
            move_count++;
        }
