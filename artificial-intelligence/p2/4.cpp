...
string astar(const vector<string>& spec, const vector<Position>& init_state, const vector<Position>& goals) {
    ...
    int h_val = better_heuristic(spec, init, memoized_for_heuristics);
    prioque.push({1.0F * h_val, 0, init, ""});
    ...
    while (!prioque.empty()) {
        ...
        for (const auto& move : next_moves) {
            ...
            int new_g = curr.g + 1;
            float h1 = better_heuristic(spec, new_st, memoized_for_heuristics) * weight_heur;
            // int h2 = heuristic(new_st, goals);
            float new_f = new_g + h1;
