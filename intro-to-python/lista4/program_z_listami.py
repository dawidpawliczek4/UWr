...
def suma_parzystych(L): #[!]
    ...
    wynik = 0
    for element in L:
        wynik += element if parzysta(element) else 0 # warunek
    return wynik
...
def histogram(L):
    ...
    for liczba in L:
        print (str(liczba) + " " * (4 - len(str(liczba))) +"*" * liczba) #[!] # 4-len razy spacje
...
def powiekszone_parzyste_z_zerami(L): #[!] drobny błąd
    ...
    for n in L:
        if parzysta(n):
            ...
            wynik.append(0)        # Znowu, mozna: wynik += [0]  / 1 tab w prawo
    return wynik      
